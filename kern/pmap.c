...
alloc_child(struct Page *parent, bool right) {
    ...
    if (right) {parent->right = new; new->addr = parent->addr + (1ULL << new->class);} 
    else {parent->left = new; new->addr = parent->addr;}
...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    for (;start != end; start += CLASS_SIZE(class)) {
        for (class = 0; class < MAX_CLASS; class++) {
            if (start & CLASS_MASK(class)) {class--; break;}
...
dump_virtual_tree(struct Page *node, int class) {
    ...
    for (int i = MAX_CLASS; i != class; i--) cprintf(" ");
    if ((node->state & NODE_TYPE_MASK) == MAPPING_NODE) cprintf("Mapping node %lx with class %d\n", (uint64_t)node->phy->addr, node->phy->class);
    else cprintf("Intermidiate node with class %d\n", class);
...
unmap_page(struct AddressSpace *spc, uintptr_t addr, int class) {
    ...
    if (class >= 9) {remove_pt(pd, addr, 2 * MB, pdi0, pdi1); goto finish;}
