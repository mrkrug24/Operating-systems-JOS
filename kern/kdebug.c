...
debuginfo_rip(uintptr_t addr, struct Ripdebuginfo *info) {
    ...
    // LAB 8: Your code here:
    if (addr >= MAX_USER_ADDRESS) load_kernel_dwarf_info(&addrs);
    else load_user_dwarf_info(&addrs);
...
find_function(const char *const fname) {
    ...
    if (!address_by_fname(&addrs, fname, &fn_offset)) return fn_offset;
    if (!naive_address_by_fname(&addrs, fname, &fn_offset)) return fn_offset;
    ...
    for (entry = ((struct Elf64_Sym *)symt_base) + 1; entry < (struct Elf64_Sym *)symt_end; entry++) 
        if (!strcmp((char *)&strt[entry->st_name], fname)) return (uintptr_t)entry->st_value;
