...
sys_env_set_status(envid_t envid, int status) {
    ...
    if (status == ENV_NOT_RUNNABLE || status == ENV_RUNNABLE) env->env_status = status;
    else return -E_INVAL;
...
sys_alloc_region(envid_t envid, uintptr_t addr, size_t size, int perm) {
    ...
    struct Env* env;
    if (envid2env(envid, &env, 1)) return -E_BAD_ENV;
    if (addr >= MAX_USER_ADDRESS || PAGE_OFFSET(addr) || perm & ~(PROT_ALL | ALLOC_ZERO | ALLOC_ONE) || perm & ALLOC_ONE & ALLOC_ZERO) return -E_INVAL;

    perm |= PROT_LAZY | PROT_USER_;
    if (perm & ALLOC_ONE) perm &= ~ALLOC_ZERO;
    else {perm |= ALLOC_ZERO; perm &= ~ALLOC_ONE;}
...
sys_map_region(envid_t srcenvid, uintptr_t srcva,
    ...
    if (envid2env(srcenvid, &srcenv, 1) || envid2env(dstenvid, &dstenv, 1)) return -E_BAD_ENV;

    if (CLASS_MASK(0) & srcva || CLASS_MASK(0) & dstva || srcva >= MAX_USER_ADDRESS || dstva >= MAX_USER_ADDRESS || perm & ~PROT_ALL || perm & ALLOC_ZERO || perm & ALLOC_ONE) return -E_INVAL;
    perm |= PROT_USER_;
...
sys_map_physical_region(uintptr_t pa, envid_t envid, uintptr_t va, size_t size, int perm) {
    ...
    struct Env * env_fs;
    if (envid2env(envid, &env_fs, true) || env_fs->env_type != ENV_TYPE_FS) return -E_BAD_ENV;
...
sys_ipc_try_send(envid_t envid, uint32_t value, uintptr_t srcva, size_t size, int perm) {
    ...
    if ((srcva < MAX_USER_ADDRESS && PAGE_OFFSET(srcva)) || (srcva < MAX_USER_ADDRESS && perm & ~PROT_ALL) || (srcva < MAX_USER_ADDRESS && (perm & PROT_W) && user_mem_check(curenv, (void *)srcva, size, PROT_W) < 0)) return -E_INVAL;
    if (!env->env_ipc_recving) return -E_IPC_NOT_RECV;
    env->env_ipc_recving = false;

    size_t actual_size = MIN(size, env->env_ipc_maxsz);
    if (srcva < MAX_USER_ADDRESS && env->env_ipc_dstva < MAX_USER_ADDRESS) {
        if (map_region(&env->address_space, env->env_ipc_dstva, &curenv->address_space, srcva, actual_size, perm | PROT_USER_)) {env->env_ipc_recving = true; return -E_NO_MEM;}
...
sys_ipc_recv(uintptr_t dstva, uintptr_t maxsize) {
    // LAB 9: Your code here
    if (PAGE_OFFSET(maxsize) || (dstva < MAX_USER_ADDRESS && (PAGE_OFFSET(dstva) || maxsize == 0))) return -E_INVAL;
    curenv->env_ipc_recving = 1;
    curenv->env_status = ENV_NOT_RUNNABLE;
    if (dstva < MAX_USER_ADDRESS) {curenv->env_ipc_dstva = dstva; curenv->env_ipc_maxsz = maxsize;}
...
sys_region_refs(uintptr_t addr, size_t size, uintptr_t addr2, uintptr_t size2) {
    ...
    if (addr2 != MAX_USER_ADDRESS) return region_maxref(&curenv->address_space, addr, size) - region_maxref(&curenv->address_space, addr2, size2);
    else return region_maxref(&curenv->address_space, addr, size);
}
