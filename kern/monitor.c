...
mon_start(int argc, char **argv, struct Trapframe *tf) {
    if (argc != 2) {cprintf("timer_start: no given name of timer.\n"); return 1;}
    timer_start(argv[1]);
    return 0;
}
...
mon_frequency(int argc, char **argv, struct Trapframe *tf) {
    if (argc != 2) {cprintf("timer_freq: no given name of timer.\n"); return 1;}
...
mon_dumpcmos(int argc, char **argv, struct Trapframe *tf) {
    ...
    for (int i = 0; i < CMOS_START + CMOS_SIZE; i += 0x10) {
        cprintf("%02x:", i);
        for (int j = i; j < i + 0x10; j++) cprintf(" %02x", cmos_read8(j));
        cprintf("\n");
    }
