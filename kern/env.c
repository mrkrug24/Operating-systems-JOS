...
load_icode(struct Env *env, uint8_t *binary, size_t size) {
    ...
    if (elf->e_magic != ELF_MAGIC) {cprintf("ELF file has magic %08X instead of %08X\n", elf->e_magic, ELF_MAGIC); return -E_INVALID_EXE;}
    if (elf->e_type != ET_EXEC) {cprintf("ELF file is not executable\n"); return -E_INVALID_EXE;}
    if (elf->e_shentsize != sizeof(struct Secthdr)) {cprintf("ELF file has sections of %u bytes instead of %lu\n", elf->e_shentsize, sizeof(struct Secthdr)); return -E_INVALID_EXE;}
    if (elf->e_phentsize != sizeof(struct Proghdr)) {cprintf("ELF file has program headers of %u bytes instead of %lu\n", elf->e_phentsize, sizeof(struct Proghdr)); return -E_INVALID_EXE;}
    if (elf->e_shstrndx >= elf->e_shnum) {cprintf("ELF file has string section index %u out of %u\n", elf->e_shstrndx, elf->e_shnum); return -E_INVALID_EXE;}
...
env_create(uint8_t *binary, size_t size, enum EnvType type) {
    ...
    // LAB 8: Your code here
    struct Env *env;
    if (env_alloc(&env, 0, type) < 0) {panic("env_alloc: can't allocate new environment.");}
    ...
    if (load_icode(env, binary, size) < 0) panic("load_icode: can't load set up new binary.");
}
