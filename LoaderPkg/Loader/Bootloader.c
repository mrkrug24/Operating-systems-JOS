  ...
  )
{
  ...
  EFI_GRAPHICS_PIXEL_FORMAT Format = PixelBlueGreenRedReserved8BitPerColor;

  for (UINT32 iter = GraphicsOutput->Mode->MaxMode - 1; iter >= 0; iter--) {
    Status = GraphicsOutput->QueryMode(GraphicsOutput, iter, &SizeOfInfo, &Info);

    if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "JOS: Cannot find mode info: %r\n", Status));
        return Status;
    }
  ...
  if (Mode == -1) {
      DEBUG ((DEBUG_ERROR, "JOS: Cannot find requested graphics mode: %r\n", Status));
      return Status;
  }
