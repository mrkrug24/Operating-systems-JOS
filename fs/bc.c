...
bc_pgfault(struct UTrapframe *utf) {
    ...
    if (res) panic("bc_pgfault: can't alloc memmory! %i", res);
    *(char *)addr = 0;
    res = nvme_read(blockno * BLKSECTS, addr, BLKSECTS);
    if (res != NVME_OK) panic("bc_pgfault on va %p failed: reading\n", addr);
...
flush_block(void *addr) {
    ...
    if (res != NVME_OK) panic("flush_block of va %p failed: writing\n", addr);
    res = sys_map_region(CURENVID, addr, CURENVID, addr, BLKSIZE, PTE_SYSCALL & get_prot(addr));
    if (res) panic("flush_block of va %p failed: clearing PTE_D\n", addr);
