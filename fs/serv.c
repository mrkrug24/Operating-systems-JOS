...
serve_read(envid_t envid, union Fsipc *ipc) {
    ...
    if (req->req_n > PAGE_SIZE) req->req_n = PAGE_SIZE;
    if ((cnt = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset)) > 0) o->o_fd->fd_offset += cnt;
    return cnt;
}
...
serve_write(envid_t envid, union Fsipc *ipc) {
    ...
    if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0) return r;
    if ((cnt = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset)) > 0) o->o_fd->fd_offset += cnt;
    return cnt;
}
